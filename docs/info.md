...
Methods and initializers are also supported similar to JS with no function keyword prefix, `this` for self-reference, and the constructor this time being `init()`
```cs
class CoffeeMaker {
  init(coffee) {
    this.coffee = coffee;
